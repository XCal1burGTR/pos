...
const NavItem = ({ id, label, icon: Icon, active, collapsed, hasBadge, pendingCreditsCount, onNavigate }) => {
    let iconContainerBg = '';
    if (!collapsed) {
    ...
    return (
        <button key={id} onClick={() => onNavigate(id)}
            ...
            {!collapsed && (
                <span className={cn('text-sm truncate transition-colors duration-200', active ? 'font-semibold' : 'font-medium')}>
                    {label}
                </span>
            )}
...
NavItem.propTypes = {
    ...
    pendingCreditsCount: PropTypes.number.isRequired,
    onNavigate: PropTypes.func.isRequired,
};

const BrandLogo = ({ isAdmin, settings, collapsed, initial }) => {
    if (isAdmin) {
        return (
            <img src={XyberixLogo} alt="Xyberix Core"
                className={cn('object-cover flex-shrink-0 transition-all duration-300 rounded-xl', collapsed ? 'h-10 w-10' : 'h-16 w-16')} />
        );
    }
    if (settings.logo) {
        return (
            <img src={settings.logo} alt="Logo"
                className={cn('rounded-2xl object-cover ring-2 ring-indigo-500/20 flex-shrink-0 transition-all duration-300 shadow-lg shadow-indigo-100/40', collapsed ? 'h-10 w-10 rounded-xl' : 'h-16 w-16')} />
        );
    }
    return (
        <div className={cn('rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center', 'text-white font-bold shadow-lg shadow-indigo-500/25 flex-shrink-0 transition-all duration-300', collapsed ? 'h-10 w-10 text-base rounded-xl' : 'h-16 w-16 text-2xl')}>
            {initial}
        </div>
    );
};
...
const Sidebar = ({ activePage, onNavigate, collapsed, setCollapsed, isMobileDrawer }) => {
    ...
    const displayName = isAdmin ? 'Xyberix Core' : storeName;
    const displaySubtitle = isAdmin ? 'Smart POS System' : 'POS System';
    ...
    return (
        <aside className={cn(
            ...
            <div className={cn(
                ...
                {/* Logo */}
                <BrandLogo isAdmin={isAdmin} settings={settings} collapsed={collapsed} initial={initial} />
                ...
                <div className={cn(
                    ...
                    <p className="font-bold text-slate-900 text-sm truncate leading-tight whitespace-nowrap">{displayName}</p>
                    <p className="text-[11px] text-slate-400 font-medium mt-0.5 whitespace-nowrap">{displaySubtitle}</p>
                </div>
            </div>
                ...
                <div className={cn("space-y-0.5", collapsed ? "px-2" : "px-2.5")}>
                    {navItems.map(({ id, label, icon }) => (
                        <NavItem
                            ...
                            collapsed={collapsed}
                            hasBadge={id === 'credit' && pendingCreditsCount > 0}
