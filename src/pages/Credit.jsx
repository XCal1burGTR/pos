...
RecordPayment.propTypes = {
    ...
    payNote: PropTypes.string.isRequired,
    setPayNote: PropTypes.func.isRequired,
...
const Credit = () => {
    ...
    // Keep selected in sync when credits update (e.g. after recording a payment)
    useEffect(() => {
        if (selected) {
            ...
            <div className="flex flex-col md:flex-row gap-3 flex-1 min-h-0 overflow-hidden">
                ...
                <div className="w-full md:w-72 flex-shrink-0 flex flex-col gap-2 min-h-0">
                    ...
                    <div className="relative flex-shrink-0">
                        ...
                        <Input placeholder="Search name, phone, order…" className="pl-9 h-9 text-sm"
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1 pr-0.5 min-h-0">
                        {filtered.length === 0 ? (
                            <p className="text-center py-8 text-sm text-slate-400">
                                {filter === 'pending' ? 'No pending credits' : 'No records found'}
                            </p>
                        ) : filtered.map(credit => (
                                ...
                                <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <div className={cn(
                                            ...
                                            credit.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                                        )}>
                                    ...
                                    <div className="text-right flex-shrink-0">
                                        <p className={cn('text-sm font-bold', credit.status === 'pending' ? 'text-amber-600' : 'text-emerald-600')}>
                                            {credit.status === 'pending' ? fmt(credit.pendingAmount) : 'Settled'}
                                        </p>
                                        <p className="text-[10px] text-slate-400">
                                            {credit.createdAt ? new Date(credit.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : ''}
                                        </p>
                                    </div>
                                </div>
                            </button>
                ...
                <div className="flex-1 min-w-0 overflow-y-auto pl-2">
                    {selected ? (
                        <div className="space-y-4 max-w-xl">
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3">
                                    <div className={cn(
                                        'h-11 w-11 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0',
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-3 mt-0.5">
                                            ...
                                            {selected.customerAddress && (
                                                <span className="text-xs text-slate-500 flex items-center gap-1">
                                                    <MapPin className="h-3 w-3" />{selected.customerAddress}
                                                </span>
                                            )}
                                            ...
                                            {selected.orderId && (
                                                <span className="text-xs text-slate-400">Order #{selected.orderId}</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <div className={cn('rounded-xl p-3 border',
                                    selected.status === 'pending' ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-200')}>
                                    <p className={cn('text-[10px] font-semibold uppercase tracking-wide',
                                        selected.status === 'pending' ? 'text-amber-500' : 'text-slate-400')}>
                                        {selected.status === 'pending' ? 'Due' : 'Cleared'}
                                    </p>
                                    <p className={cn('text-base font-bold mt-0.5',
                                        selected.status === 'pending' ? 'text-amber-700' : 'text-emerald-600')}>
                                        {selected.status === 'pending' ? fmt(selected.pendingAmount) : '₹0.00'}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="h-full flex items-center justify-center">
                            <EmptyState icon={Wallet} title="Select a record"
                                description="Choose a credit entry from the list to view details." />
                        </div>
                    )}
                </div>
            </div>
